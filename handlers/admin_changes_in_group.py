...
router = Router()
router.chat_member.filter(F.chat.id == config.chat_id)
...
async def admin_demoted(event: ChatMemberUpdated, admis: set[int]):
    admis.discard(event.new_chat_member.user.id)
    await event.answer(
        f"{event.new_chat_member.user.first_name} "
